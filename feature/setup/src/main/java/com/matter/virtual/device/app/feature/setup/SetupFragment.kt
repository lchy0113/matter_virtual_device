...
import com.matter.virtual.device.app.core.model.matter.OnboardingType
...
class SetupFragment : Fragment() {
  ...
  private fun showConfirmDialog() {
    ...
    dialogBinding.dialogSetupStartButton.setOnClickListener {
      ...
      viewModel.setCommissioningSequenceFlag()
      ...
      Timber.d("OnboardingType:$onboardingType")
      this.matterSettings.onboardingType = onboardingType
      this.matterSettings.discriminator = discriminator.toInt()

      if (onboardingType == OnboardingType.WIFI) {
        sharedViewModel.stopBusyLoop("SetupFragment") // close busy loop
      }

      try {
        findNavController()
          .navigate(
            DeepLink.getDeepLinkRequestForQrcodeFragment(Json.encodeToString(this.matterSettings))
          )
      } catch (e: Exception) {
